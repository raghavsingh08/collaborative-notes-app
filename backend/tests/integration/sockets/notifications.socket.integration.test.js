...
describe("notification socket routing", () => {
    ...
    it("removes a disconnected tab from its user room", async () => {
        ...
        const disconnectedSocketId = disconnectedTab.id
        const serverSocket = server.io.sockets.sockets.get(disconnectedSocketId)
        const serverDisconnect = new Promise((resolve) => serverSocket.once("disconnect", resolve))
        disconnectedTab.disconnect()
